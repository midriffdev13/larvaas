...
class TeamMembersRelationManager extends RelationManager
{
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->actions([
                ActionGroup::make([
                    DetachAction::make()
                        ->hidden(fn () => ! auth()->user()->can('authorization.role.teammember.detach')),
                ]),
            ])
            ->bulkActions([
    ...
    {
        return $ownerRecord->authorizationType->slug === 'team-member' && auth()->user()->can('authorization.role.teammember.view');
    }
}
