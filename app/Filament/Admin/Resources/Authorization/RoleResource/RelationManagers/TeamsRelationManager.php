...
class TeamsRelationManager extends RelationManager
{
        ...
        return $table
            ...
            ->headerActions([
                AttachTeamsAction::make()
                    ->hidden(fn () => ! auth()->user()->can('authorization.role.team.attach')),
            ])
            ->actions([
                ActionGroup::make([
                    DetachAction::make()
                        ->hidden(fn () => ! auth()->user()->can('authorization.role.team.detach')),
                ]),
            ])
            ->bulkActions([
    ...
    {
        return $ownerRecord->authorizationType->slug === 'team' && auth()->user()->can('authorization.role.team.view');
    }
}
